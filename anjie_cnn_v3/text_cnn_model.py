    ...
    def __init__(
        ...
        for i, filter_size in enumerate(filter_sizes):
            with tf.name_scope("conv-maxpool-%s" % filter_size):
                ...
                pooled = tf.nn.max_pool(
                    h,
                    ksize=[1, 1, feature_size - filter_size + 1, 1],
                    strides=[1, 1, 1, 1],
                    padding='VALID',
                    name="pool")
                pooled_outputs.append(pooled)
        ...
        with tf.name_scope("layer1"):
            ...
            W = tf.get_variable(
                "full_1_W",
                shape=[num_filters_total, num_layer1],
                initializer=tf.contrib.layers.xavier_initializer()
            )
            b = tf.Variable(tf.constant(0.1, shape=[num_layer1]), name="layer1_b")
            layer1_score = tf.nn.xw_plus_b(self.h_drop, W, b, name="layer1_scores")
            layer1_h = tf.nn.relu(tf.nn.bias_add(layer1_score, b), name="layer1_relu")
            layer1_drop = tf.nn.dropout(layer1_h, self.dropout_keep_prob)
        ...
        with tf.name_scope("output"):
            ...
            W = tf.get_variable(
                "W",
                shape=[num_layer1, num_classes],
                initializer=tf.contrib.layers.xavier_initializer()
            )
            b = tf.Variable(tf.constant(0.1, shape=[num_classes]), name="b")
            l2_loss += tf.nn.l2_loss(W)
            l2_loss += tf.nn.l2_loss(b)
            self.scores = tf.nn.xw_plus_b(layer1_drop, W, b, name="scores")
            self.predictions = tf.argmax(self.scores, 1, name="predictions")
...
class TextCNN_TFIDF_Weighted(object):
    """
    A CNN for text classification.
    Uses an embedding layer, followed by a convolutional, max-pooling and softmax layer.
    """
